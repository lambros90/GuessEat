...
public class Lava extends Objects {
    ...
    public Lava(SecondStage screen, float x, float y) {
        super(screen, x, y);
        frames = new Array <TextureRegion>();
        for(int i = 0; i < 3; i++){
            frames.add(new TextureRegion(screen.getAtlasLava().findRegion("10008"), i * 32, 2, 32, 32));
        }
    ...
    protected void defineObjects() {
        ...
        vertice[0] = new Vector2(-2, 8).scl(1 / GuessEat.PPM);
        vertice[1] = new Vector2(2, 8).scl(1 / GuessEat.PPM);
        vertice[2] = new Vector2(2, -8).scl(1 / GuessEat.PPM);
        vertice[3] = new Vector2(-2, -8).scl(1 / GuessEat.PPM);
        shape.set(vertice);
        fdef.shape = shape;
